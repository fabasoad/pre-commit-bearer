...
parse_all_args() {
  ...
  while [[ -n "${args}" ]]; do
    case "$(echo "${args}" | cut -d '=' -f 1)" in
      ...
      *)
        ...
        elif [ "${curr_flag}" = "bearer" ]; then
          args_map_ref["bearer-args"]="${args_map_ref["bearer-args"]} ${arg}"
        else
          msg="Invalid format of the following argument: \"${arg}\". Please use"
          msg="${msg} --hook-args to pass args to pre-commit hook or --bearer-args"
          msg="${msg} to pass args to bearer. For more information see https://github.com/fabasoad/pre-commit-bearer?tab=readme-ov-file"
  ...
  args_map_ref["bearer-args"]=$(echo "${args_map_ref["bearer-args"]}" | sed 's/^ *//')
}
