...
PRE_COMMIT_BEARER_SRC_DIR=$(dirname "$(realpath "$0")")

# Import all scripts
_import_all() {
  current_file=$(realpath "$0")
  exec_files=$(find "${PRE_COMMIT_BEARER_SRC_DIR}" -type f \( -perm -u=x -o -perm -g=x -o -perm -o=x \))
...
main() {
  ...
  apply_bearer_config "${hook_args_map["${CONFIG_BEARER_VERSION_ARG_NAME}"]}"
